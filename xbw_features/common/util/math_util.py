...
def getNumFromStr(numStr):
    num = int(re.findall("\\d+", numStr)[0])
    return num


def get_diff_points(list_a, list_b):
    a_set = {(
     p.x(), p.y()) for p in list_a}
    return [p for p in list_b if (p.x(), p.y()) not in a_set]


def filter_similar_points(points, threshold=20, min_group_num=3):
    used = set()
    result = []
            ...
            for j, p2 in enumerate(point_list):
                if not j in used:
                    if i == j:
                        ...
                        if distance_between_points(p1, p2) < threshold:
                            group.append(p2)
                            used.add(j)
