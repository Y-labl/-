...
import { pointCardPointsToYuan } from '../../utils/bizDate';
import {
  BIZ_DATE_PAGE,
  getLedgerBizDate,
  lockLedgerBizDate,
...
export default function MechanicalLedgerPage() {
  ...
  const todayLinesSyncChainRef = useRef<Promise<void>>(Promise.resolve());

  const queueSyncTodayLines = useCallback(
    (
      lines: TodayLine[],
      opts?: { toastOnError?: boolean; bizDateOverride?: string; rethrowOnError?: boolean },
    ): Promise<void> => {
      const payload = lines.map((l) => ({ name: l.name, valueW: l.valueW, count: l.count }));
      const targetBizDate = opts?.bizDateOverride ?? getLedgerBizDate();
      const next = todayLinesSyncChainRef.current
        .catch(() => {})
        .then(() => api.mechLedgerPutTodayLines({ bizDate: targetBizDate, lines: payload }))
        .then(() => {});
      todayLinesSyncChainRef.current = next.catch(() => {});
      return next.catch((e) => {
        if (opts?.toastOnError) {
          setToast(e instanceof Error ? e.message : '物品列表同步失败，请检查网络或是否已登录');
  ...
  const scheduleSyncTodayLines = useCallback(
    (lines: TodayLine[]) => {
      void queueSyncTodayLines(lines, { toastOnError: true });
    },
    [queueSyncTodayLines],
  );

  const flushSyncTodayLines = useCallback(
    async (lines: TodayLine[]) => {
      await queueSyncTodayLines(lines);
    },
    [queueSyncTodayLines],
  );

  const forceSyncTodayLines = useCallback(
    async (lines: TodayLine[]) => {
      await queueSyncTodayLines(lines, { rethrowOnError: true });
    },
    [queueSyncTodayLines],
  );
  ...
  /** 当前页离开时，等待已排队的 today-lines 写库完成 */
  const flushPendingSync = useCallback(() => {
    const lines = todayLinesRef.current;
    return queueSyncTodayLines(lines);
  }, [queueSyncTodayLines]);
  ...
  useEffect(() => {
    ...
    const lines = todayLinesRef.current;
    void queueSyncTodayLines(lines, { bizDateOverride: prev });
    flushPendingMetaSaveRef.current(prev, { force: true });
    // 未锁定时全站业务日可能自动从「昨天」跳到「今天」，物品已写入 prev；不提示易被误认为丢失
    if (prev < cur && lines.length > 0) {
      setToast(
        `业务日已从 ${prev} 变为 ${cur}：${lines.length} 条物品已写入 ${prev}。要看这些内容请在上方日期选回 ${prev}。`,
      );
      window.setTimeout(() => setToast(null), 5200);
    }
  }, [bizDate, queueSyncTodayLines]);
  ...
  const addTodayItem = useCallback(
    async (item: LedgerItemDef) => {
      ...
      const prev = todayLinesRef.current;
      ...
      try {
        await forceSyncTodayLines(next);
        setTodayLines(next);
      } catch (e) {
        setToast(e instanceof Error ? e.message : '物品保存失败，请检查网络或是否已登录');
        window.setTimeout(() => setToast(null), 2800);
      }
    },
    [forceSyncTodayLines]
  ...
  const confirmPickModal = useCallback(() => {
    ...
    void addTodayItem({ ...item, name, valueW: w });
    setPickModal(null);
  ...
  const onRewardIcon = useCallback(
    (item: LedgerItemDef) => {
      bumpYaksha('y1', 1);
      void addTodayItem({ ...item, valueW: exactPrice ? item.valueW * 1.2 : item.valueW });
    },
  ...
  const commitLineUnitPrice = useCallback(
    async (lineId: string) => {
      ...
      if (idx < 0) return;
      if (Math.abs(Number(prev[idx].valueW) - n) < 1e-9) {
      ...
      if (!row && name) {
        const wanted = normalizeCatalogLookupName(name);
        row =
          flat.find((r) => normalizeCatalogLookupName(r.name) === wanted) ||
          flat.find((r) => normalizeCatalogLookupName(r.name).includes(wanted) || wanted.includes(normalizeCatalogLookupName(r.name)));
      }
      if (!row) {
  ...
  const persistDaySnapshot = useCallback(async (snapshotBizDate: string) => {
    ...
    await queueSyncTodayLines(todayLines, { bizDateOverride: snapshotBizDate });
  ...
  const confirmRemoveLineQty = useCallback(() => {
    ...
    if (rm >= removeLineTarget.count) {
      void removeLine(removeLineTarget.id);
    } else {
      void decreaseLineCount(removeLineTarget.id, rm);
    }
