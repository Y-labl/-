...
ledgerRouter.put('/mech-ledger/today-lines', async (req, res) => {
  ...
  try {
    ...
    const verifyCount = Array.isArray(verifyRows) ? verifyRows.length : 0;
    if (verifyCount !== lines.length) {
      return res.status(503).json({
        error: '记账台保存后回读校验失败，请检查数据库连接或表结构',
      });
    }
...
ledgerRouter.post('/mech-ledger/save-day', async (req, res) => {
  ...
  try {
    ...
    const [verifyRows] = await pool.query(
      `SELECT point_card_points AS pointCardPoints, online_roles AS onlineRoles,
              elapsed_sec AS elapsedSec, cash_game_gold_w AS cashGameGoldW,
              team_principals_w AS teamPrincipalsW, team_cash_game_gold_w AS teamCashGameGoldWRaw
       FROM mech_ledger_day_meta WHERE user_id = ? AND biz_date = ? LIMIT 1`,
      [uid, bizDate]
    );
    const vr = verifyRows?.[0];
    if (!vr) {
      return res.status(503).json({ error: '保存收益后回读失败，请检查数据库连接或表结构' });
    }
    const vrPoint = Number(vr.pointCardPoints ?? 0);
    const vrOnline = Number(vr.onlineRoles ?? 0);
    const vrElapsed = vr.elapsedSec == null ? null : Number(vr.elapsedSec);
    const vrCash = Number(vr.cashGameGoldW ?? 0);
    const vrPrincipals = parseTeamPrincipalsColumn(vr.teamPrincipalsW);
    const vrCashTeams = parseTeamPrincipalsColumn(vr.teamCashGameGoldWRaw);
    const matchPoint = Math.abs(vrPoint - pointCardPoints) < 1e-9;
    const matchOnline = vrOnline === onlineCount;
    const matchElapsed = (vrElapsed == null && elapsedSec == null) || vrElapsed === elapsedSec;
    const matchCash = Math.abs(vrCash - cashGold) < 1e-9;
    const matchPrincipals = JSON.stringify(vrPrincipals) === principalsJson;
    const matchTeamCash =
      (!teamCashJson && (!Array.isArray(vrCashTeams) || vrCashTeams.length === 0)) ||
      JSON.stringify(vrCashTeams) === teamCashJson;
    if (!(matchPoint && matchOnline && matchElapsed && matchCash && matchPrincipals && matchTeamCash)) {
      return res.status(503).json({ error: '保存收益后数据校验不一致，请刷新后重试' });
    }
...
ledgerRouter.get('/mech-ledger/daily', async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      if (Array.isArray(shadowRows) && shadowRows.length > 0) {
        rows.push(...shadowRows);
      }
    }
