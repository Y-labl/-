 ...
 * 运行时读写在内存 `prefsInternal`，变更经 `patchClientPrefs` 防抖 PUT。
 */
import { api } from '../api';
...
export function patchClientPrefs(partial: Partial<UserClientPrefs>): void {
  prefsInternal = mergePrefs(getClientPrefsSnapshot(), partial);
  notifyAll();
  scheduleSave();
}
...
function scheduleSave(): void {
  ...
  saveTimer = setTimeout(() => {
    ...
    void api.clientPrefsPut({ prefs: payload as Record<string, unknown> }).catch(() => {});
  }, 400);
}
...
export function flushClientPrefsNow(): Promise<void> {
  ...
  const payload = getClientPrefsSnapshot();
  return api
    .clientPrefsPut({ prefs: payload as Record<string, unknown> })
    .then(() => {})
    .catch(() => {});
}
...
function sweepLegacyBrowserStorage(): void {
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const k = localStorage.key(i);
      if (!k || k === BROWSER_TOKEN_KEY) continue;
