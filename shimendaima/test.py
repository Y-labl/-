...
def press_f9_and_click_image(image_path, confidence=0.85):
    """
    按下 F9 键，然后识别图片并点击其中心位置。
    ...
    try:
        ...
        # 识别图片并获取中心位置
        center_position = locate_image_center(image_path, confidence)
        if not center_position:
            logging.error(f"未能找到图片 '{image_path}'，程序终止。")
            return None
...
def main():
    ...
    time.sleep(0.7)  # 等待 F9 键的效果

    rw_position = locate_image_center("../shimen/renwu.png", target_window)

    # 点击图片的中心位置
    click_image_center(rw_position)
