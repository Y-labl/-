# Decompiled from Python 3.8 bytecode
# Source: map_action.pyc
...
class AreaParams:
    __module__ = __name__
    __qualname__ = "AreaParams"

    def __init__(self, area, isFlyMap, isLightFunc, mapLeftBottomPoint, xyWidthHeight, xyRange):
        self.area = area
...
def goToMapAction(deviceId, area, flyMapXY=None, curTryT=0):
    """Complete map navigation with special routes."""
    try:
        ...
        if mapParams and mapParams.isFlyMap:
            ...
            if not isArrive:
                doubleClickProduct(deviceId, preImgName="飞行符", preFunc=lambda: True, nextImgName="使用飞行符结果")
                clickFlyMap(deviceId, preImgName="飞行符飞" + area, nextFunc=lambda: detectPosition(deviceId)[0] == area)
            return None
...
def clickNpcAction(deviceId, mapParams, npcPoint, clickPopImgName=None,
...
def _feiXingQi(deviceId,area,flyMapXY):
    pass  # decompile failed: 404 bytes
...
def _doHideLocation(deviceId):
    ...
    findPics(deviceId, imgNames=["��ѡ��"], withClick=True)
    ...
    for i in range(2):
        ...
        if lastFrame is not None:
            if isframeSame(curFrame, lastFrame, similar=0.98):
                ...
                logger.info(f"{deviceId}图没晃动了")
                break
            else:
                logger.info(f"{deviceId}图晃动了")
        lastFrame = curFrame
        time.sleep(0.2)
    else:
