...
Page({
  ...
  /** 含当前考勤月在内共 3 个考勤月（21 日～次月 20 日）的最早一日 0 点；无则退回今天 0 点 */
  getCutoffRetainThreeAttendanceMonths() {
    const since = app.getAttendanceDataRetainSinceDate(new Date(), 3);
    if (since) {
  ...
  /** 仅保留 cutoff（含）之后的点数记录；设置、账本、模板等不动 */
  trimRecordsKeepLastThreeMonths() {
    const keys = app.globalData.STORAGE_KEYS;
    const cutoff = this.getCutoffRetainThreeAttendanceMonths();
    const records = app.getRecords();
    const next = {};
    let removed = 0;

    Object.keys(records).forEach((dateStr) => {
      const t = app.parseDateStrLocal(dateStr);
      if (Number.isNaN(t.getTime())) {
        return;
      }
      t.setHours(0, 0, 0, 0);
      if (t >= cutoff) {
        next[dateStr] = records[dateStr];
      } else {
        removed++;
      }
    });

    wx.setStorageSync(keys.RECORDS, next);
    this.trimAuxStorageByCutoff(keys.GOALS, cutoff);
    this.trimAuxStorageByCutoff(keys.REMINDERS, cutoff);

    this.loadSettings();
    const msg = removed > 0 ? `已删除 ${removed} 天更早记录` : '没有更早的记录';
    wx.showToast({ title: msg, icon: removed > 0 ? 'success' : 'none' });
  },

  /**
   * 对以 yyyy-MM-dd 或 yyyy-MM 为键的简单对象做裁剪（无此类键则原样写回）
   */
  trimAuxStorageByCutoff(storageKey, cutoff) {
    ...
    Object.keys(raw).forEach((k) => {
      ...
      if (/^\d{4}-\d{2}-\d{2}$/.test(k)) {
        t = app.parseDateStrLocal(k);
      } else if (/^\d{4}-\d{2}$/.test(k)) {
        const parts = k.split('-');
        t = new Date(parseInt(parts[0], 10), parseInt(parts[1], 10) - 1, 1);
      }
  ...
  /** 全部清除前的二次确认 */
  confirmFullClearStorage() {
    wx.showModal({
      title: '再次确认',
      content: '将清除本机全部数据（含设置、账本、模板、备份相关键等），且无法恢复。确定继续？',
      confirmText: '确定清除',
      confirmColor: '#ff4d4f',
      success: (res2) => {
        if (res2.confirm) {
  ...
  clearData() {
    wx.showModal({
      title: '清空数据',
      content:
        '「保留最近三个月」：按考勤月（上月21日～本月20日）连续保留含本月在内的 3 个周期，更早的点数记录会删除（及同范围的历史目标/提醒键）；设置与模板等保留。「全部清除」：抹掉本机全部数据，不可恢复。',
      cancelText: '保留最近三个月',
      confirmText: '全部清除',
      confirmColor: '#ff4d4f',
      success: (res) => {
        if (res.confirm) {
          this.confirmFullClearStorage();
        } else if (res.cancel) {
          this.trimRecordsKeepLastThreeMonths();
        }
      }
    });
  },
